<div align="center">
  ...
  <img src="https://img.shields.io/badge/version-v1.0.5a-blue" />
...
- ✨ **自动推送可选附带总结** - 新增 `auto_push_summary` 配置项，关闭后仅推送视频基本信息（封面、标题、UP主、简介、链接等），不消耗 LLM 额度
- 📝 优化 `enable_auto_push` 文案描述
- 🐛 **修复引用小程序/链接消息再次触发自动总结** - `on_all_message` 入口前置两道硬拦截，任何带 `[CQ:reply`、`[引用消息]` 标识或 reply/quote 组件的消息直接放弃自动识别

> 致谢 [@Jeric-X](https://github.com/Jeric-X) 的 PR [#18](https://github.com/storyAura/astrbot_plugin_biliVideo/pull/18)

## 🎉 历史更新 (v1.0.4b)

- 🐛 **修复字幕路径下总结后崩溃** - 仅使用平台字幕、不下载音频时，结尾误访问 `audio_meta.file_path` 触发 `NoneType` 异常，导致 AI 已生成的总结变成"生成失败"
- 🛡️ **增强清理函数健壮性** - `_cleanup` 增加空值判断，传入 `None` 或空路径时安全跳过
...
**3. 安装 wkhtmltopdf（开启图片输出时需要）**

> ⚠️ `wkhtmltopdf` 是**系统级程序**，不是 Python 包。`pip install imgkit` 只是 Python 调用层，实际渲染需要系统中安装 `wkhtmltopdf` 可执行文件。
...
如果提示 `command not found`，请根据你的环境安装：

<details>
<summary><b>🐧 Linux（宿主机直接部署）</b></summary>

```bash
# Ubuntu/Debian
apt update && apt install -y wkhtmltopdf fonts-wqy-zenhei
...
<details>
<summary><b>🐳 Docker 容器内部署（常见问题场景）</b></summary>

Docker 容器通常缺少图形环境和中文字体，需要额外安装：

```bash
# 进入容器
docker exec -it <容器名> bash

# 安装 wkhtmltopdf + 虚拟显示 + 中文字体
apt update && apt install -y wkhtmltopdf xvfb fonts-wqy-zenhei fonts-noto-cjk

# 创建 xvfb 包装脚本（解决无图形环境问题）
echo '#!/bin/bash
xvfb-run --auto-servernum /usr/bin/wkhtmltoimage "$@"' > /usr/local/bin/wkhtmltoimage
chmod +x /usr/local/bin/wkhtmltoimage

# 验证安装
echo '<h1>测试中文渲染</h1>' | xvfb-run --auto-servernum wkhtmltoimage - /tmp/test.png && echo "✅ 成功" || echo "❌ 失败"
```

**说明：**
- `xvfb` — 虚拟帧缓冲，wkhtmltopdf 需要图形环境才能渲染
- `fonts-wqy-zenhei` / `fonts-noto-cjk` — 中文字体，否则中文显示为方块
- 包装脚本让 `imgkit` 调用时自动使用 `xvfb-run`

> 💡 如果你使用自定义 Dockerfile，建议将上述命令写入 Dockerfile 中持久化。
...
2. 安装后确保 `wkhtmltoimage.exe` 所在目录已添加到系统 `PATH`
3. 验证：在 CMD 中运行 `wkhtmltoimage --version`

</details>

> 💡 **不想安装 wkhtmltopdf？** 在插件配置中将 `output_image` 设为 `false`，插件会直接输出纯文本总结，无需图片渲染依赖。
...
- ✨ **Optional summary on auto-push** - New `auto_push_summary` config option. Disable to push only basic video info (cover, title, uploader, description, link), saving LLM credits
- 📝 Refined wording for `enable_auto_push`
- 🐛 **Fix quoted mini-app/link messages re-triggering auto summary** - Two early intercepts in `on_all_message`: any message containing `[CQ:reply`, `[引用消息]` markers, or reply/quote components is now skipped from auto detection

> Thanks to [@Jeric-X](https://github.com/Jeric-X) for PR [#18](https://github.com/storyAura/astrbot_plugin_biliVideo/pull/18)

## 🎉 Previous Update (v1.0.4b)

- 🐛 **Fix crash after summarization on subtitle-only path** - When using platform subtitles without downloading audio, the trailing `audio_meta.file_path` access raised `NoneType` AttributeError, turning a successful AI summary into a "generation failed" message
- 🛡️ **Harden cleanup function** - `_cleanup` now safely skips when `file_path` is `None` or empty
